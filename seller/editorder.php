...
if (!isset($_SESSION['seller_login'])) {
	...
	if (isset($_REQUEST['eoid'])) {
		...
		$getposts5 = mysqli_query($conn,"SELECT * FROM orders WHERE id='$eoid'") or die(mysqli_error());
	...
	$user = $_SESSION['seller_login'];
	$result = mysqli_query($conn,"SELECT * FROM seller WHERE id='$user'");
...
$getposts = mysqli_query($conn,"SELECT * FROM products WHERE id ='$eopid'") or die(mysqli_error());
