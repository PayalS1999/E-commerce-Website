...
$_POST['last_name'] = trim($_POST['last_name']);
	try {
		...
		if(empty($_POST['admintype'])) {
			throw new Exception('Seller Type can not be empty');
		...
		$check = 0;
		$e_check = mysqli_query($conn,"SELECT email FROM `seller` WHERE email='$u_email'");
			...
			if ($check == 0 ) {
				if ($email_check == 0) {
					if (strlen($_POST['password']) >4 ) {
						...
						$result = mysqli_query($conn,"INSERT INTO seller (firstName,lastName,email,mobile,address,password,type,confirmCode) VALUES ('$_POST[first_name]','$_POST[last_name]','$_POST[email]','$_POST[mobile]','$_POST[signupaddress]','$_POST[password]','$_POST[admintype]','$confirmCode')");
						
						//success message
						$success_message = '
						<div class="signupform_content"><h2><font face="bookman">Seller Registration Successfull!</font></h2>
...
<html>
	<head>
		<title>Welcome to Kalakriti</title>
										...
										<div>
											<form action="" method="POST" class="registration">
												<div class="signup_form">
													...
													<div>
														<input name="signup" class="uisignupbutton signupbutton" type="submit" value="Add Seller!">
													</div>
